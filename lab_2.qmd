...
Suppose we collected onset-to-report delay data from 100 cases during an ongoing epidemic. The frequency of these delays, measured in days, is as follows:
```{r}
delays <- rlnorm(100, log(4), 1)
hist(delays, breaks = 10,
     col = "lightgreen",
     main = "Histogram of Delays",
     xlab = "Delay (days)",
     ylab = "Frequency"
     )
```
We can now estimate the probability distribution of these delays to better understand their underlying pattern as follows:
```{r}
#| message: false
#| warning: false
onset_to_report <-EpiNow2::estimate_delay(delays,
                                  samples = 1000, 
                                  bootstraps = 2)
plot(onset_to_report)
print(onset_to_report$parameters)
```
...
We measure disease severity in terms CFR. The CFR is interpreted as the conditional probability of death given confirmed diagnosis, calculated as the ratio of the cumulative number of deaths $D_{t}$ to the cumulative number of confirmed cases $C_{t}$ at a certain time $t$. We can refer to the _naive CFR_ (also crude or biased CFR, $b_{t}$):
...
Using `{cfr}`, we can change the inputs for the numerator (`deaths`) and denominator (`cases`) to estimate more severity measures like the Infection fatality risk (IFR) or the Hospitalisation Fatality Risk (HFR). We can follow this analogy:
